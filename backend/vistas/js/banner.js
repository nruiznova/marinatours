...
$("input[name='subirBanner'], input[name='editarBanner']").change(function(){
  ...
  // }else{
    ...
    $(datosImagen).on("load", function(event){
      ...
      $("#newBanner").val(rutaImagen)

      if(imagen["type"] != "video/mp4"){

        $(".previsualizarBanner").show()

        $(".previsualizarBanner").attr("src", rutaImagen);   
        
        $(".previsualizarVideo").hide()

      }else{

        $(".previsualizarVideo").show()

        $(".previsualizarBanner").hide() 
...
$(document).on("click", ".editarBanner", function(){
  ...
  $.ajax({
    ...
    success:function(respuesta){
      ...
      if(respuesta["img"].indexOf("mp4") > -1){

        $('.previsualizarVideo').attr("src", respuesta["img"]);
